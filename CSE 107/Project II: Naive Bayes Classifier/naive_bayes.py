...
# Student Name: Anya Rajan
# UW Email    : arajan2@ucsc.edu
...
class NaiveBayes():
    ...
    def fit(self, train_hams:list, train_spams:list):
        ...
        def get_counts(filenames:list):
            ...
            #pass
            #for i in filenames:
            #    for x in (self.word_set(filenames)):
            #        self.filenames(x) = 
        ...
        for i in range(len(train_hams)):
            a_set = self.word_set(train_hams[i])
            for x in (a_set):
                #self.num_train_hams += 1
                self.word_counts_ham[x] = self.word_counts_ham.get(x, 0) + 1

        for i in range(len(train_spams)):
            b_set = self.word_set(train_spams[i])
            for x in (b_set):
                #self.num_train_spams += 1
                self.word_counts_spam[x] = self.word_counts_spam.get(x, 0) + 1

        #get_counts(train_hams)
        #get_counts(train_spams)

        #print(self.word_counts_spam)
        #print(self.word_counts_ham)
    ...
    def predict(self, filename:str):
        ...
        total_spam_prob = self.num_train_spams/(self.num_train_hams + self.num_train_spams) # don't need to do laplace
        total_ham_prob = self.num_train_hams/(self.num_train_hams + self.num_train_spams)  # don't need to do laplace

        word_probs_spam = 0
        word_probs_ham = 0

        filename_set = self.word_set(filename)
        for x in (filename_set):
            word_probs_spam += np.log((self.word_counts_spam.get(x, 0) + 1)/(self.num_train_spams + 2))

        for x in (filename_set):
            word_probs_ham += np.log((self.word_counts_ham.get(x, 0) + 1)/(self.num_train_hams + 2))

        final_spam = word_probs_spam + np.log(total_spam_prob)
        final_ham = word_probs_ham + np.log(total_ham_prob)
